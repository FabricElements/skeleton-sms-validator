 ...
 within third-party archives. Copyright 2016 Chad Selph
...
const util = {
  ...
  isHighSurrogate: function(c) {
    let codeUnit = c.charCodeAt != undefined ? c.charCodeAt(0) : c;
  ...
  */
  unicodeCharacters: function(string) {
    let chars = util.map(string, util.id);
    let result = [];
  ...
  */
  unicodeCodePoints: function(string) {
    let charCodes = util.map(string, function(x) {
      return x.charCodeAt(0);
    });
    let result = [];
    while (charCodes.length > 0) {
      if (util.isHighSurrogate(charCodes[0])) {
        let high = charCodes.shift();
        let low = charCodes.shift();
  ...
  */
   ...
   * Segment with
   *
   * @param {number} maxSingleSegmentSize
   * @param {number} maxConcatSegmentSize
   * @param {Function} doEncode
   * @return {*}
  */
  _segmentWith: function(maxSingleSegmentSize, maxConcatSegmentSize, doEncode) {
    return function(listOfUnichrs) {
      let bytes = util.map(listOfUnichrs, doEncode);
      ...
      while (listOfUnichrs.length > 0) {
        let segment = {text: [], bytes: []};
        let length = 0;
        function nextChrLen() {
          return bytes[0] === undefined ? length : length + bytes[0].length;
        }
        while (
          listOfUnichrs.length > 0 &&
          nextChrLen() <= maxConcatSegmentSize
        ) {
          let c = listOfUnichrs.shift();
          let b = bytes.shift();
          segment.text.push(c);
